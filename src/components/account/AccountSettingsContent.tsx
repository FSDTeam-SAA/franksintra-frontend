...
export function AccountSettingsContent({
  ...
  const isEmbedded = !showAppHeader
  ...
  return (
    <div
      className={cn(
        'text-[15px] text-slate-900 md:text-base',
        isEmbedded ? 'bg-transparent' : 'min-h-screen bg-slate-50',
      )}
    >
      ...
      <main
        className={cn(
          isEmbedded
            ? 'w-full'
            : 'mx-auto max-w-7xl px-3 py-4 sm:px-4 md:px-6 md:py-6',
        )}
      >
        <div
          className={cn(
            'grid items-start grid-cols-1 lg:grid-cols-[1.15fr_0.85fr]',
            isEmbedded ? 'gap-4' : 'gap-6',
          )}
        >
          {/* Left Column: Profile & Address Details */}
          <div className={cn(isEmbedded ? 'space-y-4' : 'space-y-6')}>
            <Card className="overflow-hidden rounded-[1.75rem] border-slate-200 bg-white shadow-sm">
              <CardHeader
                className={cn(
                  'border-b border-slate-100 bg-[linear-gradient(135deg,_#f8fbff_0%,_#eef4ff_100%)]',
                  isEmbedded && 'p-4 sm:p-5',
                )}
              >
                <div
                  className={cn(
                    'inline-flex items-center justify-center rounded-2xl bg-[#4285F4]/10 text-[#4285F4]',
                    isEmbedded ? 'h-10 w-10' : 'h-12 w-12',
                  )}
                >
                  <UserRound className="h-5 w-5" />
                </div>
                <CardTitle className={cn('mt-3', isEmbedded ? 'text-xl' : 'text-2xl')}>
                  Profile Settings
                </CardTitle>
              ...
              <CardContent className={cn('p-4 sm:p-6', isEmbedded && 'sm:p-5')}>
                {/* ── Avatar Upload ── */}
                <div
                  className={cn(
                    'flex items-center rounded-2xl border border-slate-200 bg-slate-50/60 p-4',
                    isEmbedded ? 'mb-4 gap-4' : 'mb-6 gap-5',
                  )}
                >
                  <div className="relative shrink-0">
                    ...
                    {isUploadingAvatar && (
                      <div className="absolute inset-0 rounded-2xl bg-black/35 p-3">
                        <Skeleton className="h-full w-full rounded-xl bg-white/35" />
                      </div>
                    )}
                  </div>
          ...
          <div className={cn(isEmbedded ? 'space-y-4' : 'space-y-6')}>
            ...
            <Card className="overflow-hidden rounded-[1.75rem] border-slate-200 bg-white shadow-sm">
              ...
              <CardContent className={cn('space-y-4 p-4 sm:p-6', isEmbedded && 'sm:p-5')}>
            ...
            <Card className="rounded-[1.75rem] border-slate-200 bg-white shadow-sm">
              ...
              >
                ...
                <CardTitle className={cn('mt-3', isEmbedded ? 'text-lg' : 'text-xl')}>
                  Change Password
                </CardTitle>
